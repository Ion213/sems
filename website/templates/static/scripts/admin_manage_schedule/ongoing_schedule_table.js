...
$(document).ready(function () {
    ongoing_sched_table = new DataTable('#ongoing_sched_table', {
                ...
                buttons: [
                    {
                        ...
                        action: function () {
                            let selectedIds = $('.select-item-ongoing:checked').map(function() {
                                return $(this).data('id');
                            }).get();
                                ...
                                if (result.isConfirmed) {
                                    $.ajax({
                                        ...
                                        success: function(response) {
                                            if (response.success) {
                                                $('.select-item-ongoing:checked').closest('tr').each(function() {
                                                    ongoing_sched_table.row($(this)).remove().draw();
                                                });
                                                Swal.fire({
                                                    icon: 'success',
                                                    title: 'Deleted!',
        ...
        //table fetch data
        ajax: '/schedule_data_ongoing', 
        columns: [
            { data: 'id',
                render: function(data, type, row) {
                    return `<input type="checkbox" class="select-item-ongoing" data-id="${data}">`;
                },
                title: '<input type="checkbox" id="selectAllongoing">'
            },
            { data: 'name' },
            { data: 'scheduled_date'},
        ...
        $(document).on("click", "#selectAllongoing", function() {
            // Toggle all checkboxes in the table
            var isChecked = $(this).prop('checked');
            $(".select-item-ongoing").prop('checked', isChecked);
            ongoing_sched_table.rows().select(isChecked); // Select all rows if checked
        });
